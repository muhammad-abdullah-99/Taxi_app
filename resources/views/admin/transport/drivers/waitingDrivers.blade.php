...
<div class="page-title">
    <div class="row">
        <div class="col-sm-6">
            <h4 class="mb-0"> قائمة السائقين </h4>
        </div>
        <div class="col-sm-6">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('editDriver', $driver->id) }}" class="btn btn-info btn-sm">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    @endif
                                    <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#messageModal{{ $driver->id }}">
    إرسال رسالة
</button>
<!-- مودال إرسال الرسالة -->
<div class="modal fade" id="messageModal{{ $driver->id }}" tabindex="-1" role="dialog" aria-labelledby="messageModalLabel{{ $driver->id }}" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form action="{{ route('sendDriverMessage') }}" method="POST">
      @csrf
      <input type="hidden" name="driver_id" value="{{ $driver->id }}">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="messageModalLabel{{ $driver->id }}">إرسال رسالة للسائق</h5>
          ...
          <button type="submit" class="btn btn-success">إرسال</button>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('archiveDriver', $driver->id) }}" method="post">
                                            @csrf
                                            <div class="modal-body">
                                                <h4>هل أنت متاكد علي استبعاد السائق؟؟</h4>

                                                <div class="form-group mt-3">
                                                    <label for="lang">اختر اللغة لإرسال الرسالة:</label>
                            ...
                            <div class="modal fade" id="active{{ $driver->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title"> الموافقة علي السائق </h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <form action="{{ route('activeDriver', $driver->id) }}" method="post">
                                            @csrf
                                            <div class="modal-body">
                                                <h4>هل أنت متاكد علي الموافقه علي السائق؟؟</h4>
