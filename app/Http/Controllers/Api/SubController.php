...
function createSubscription(Request $request)
{
    try {
        ...
        // ✅ MODIFICATION: CHECK IF EXISTING INACTIVE SUBSCRIPTION EXISTS
    ...
    {
        $subscriptions = Subscription::where('user_id', $driver)->where('status', 'active')->where('expire_at', '>=', Carbon::today())->whereHas('package')->with('package')->get();
